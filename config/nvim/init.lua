...
require('notify').setup({
    stages = "slide", -- Анимация появления/исчезновения
    timeout = 1000, -- Время отображения уведомлений (в миллисекундах)
    render = "minimal", -- Более минималистичный стиль
    max_width = 30, -- Максимальная ширина уведомлений
    max_height = 10, -- Максимальная высота уведомлений
})
vim.notify = require('notify') -- Установить notify как стандартный обработчик уведомлений
...
require('lualine').setup {
  options = {
    icons_enabled = false,          -- Отключение иконок
    theme = 'everforest',                 -- Автоматический выбор темы
    component_separators = 'I',     -- Разделитель между компонентами
    section_separators = '',        -- Отсутствие секций треугольников
    always_divide_middle = true,    -- Равномерное разделение
  },
  sections = {
    lualine_a = {'mode'},
    lualine_b = {'branch', 'diff', 'diagnostics'},
